...
void ABombermanCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("SpawnBomba", IE_Pressed, this, &ABombermanCharacter::SpawnBomba);

}
void ABombermanCharacter::SpawnBomba()
{
	UE_LOG(LogTemp, Warning, TEXT("SpawnBomba() ha sido llamado!"));
	//verefica si se puede generar una bomba
    if (BombasActivas < MaxBombas)  
    {
        ...
        Abomba* NuevaBomba = GetWorld()->SpawnActor<Abomba>(BombaClase, posicionBomba, FRotator::ZeroRotator);
		//esto verifica si la bomba fue creada correctamente
        if (NuevaBomba)
        {
			//ise incrementa , indicando que ya hay una bomba adicional en el juego 
            BombasActivas++;
            NuevaBomba->SetOwner(this);
...
void ABombermanCharacter::BombaExplotada()
{
